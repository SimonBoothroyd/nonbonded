...
from nonbonded.library.statistics.statistics import compute_statistics, StatisticType
...
def test_rmse():
    ...
    estimated_values = numpy.random.normal(
        0.0, expected_std, N_DATA_POINTS
    )
    ...
    assert numpy.isclose(
        statistic_values[StatisticType.RMSE], expected_std, rtol=0.1
    )
...
def test_r2():
    ...
    assert numpy.isclose(
        statistic_values[StatisticType.R2], 1.0, rtol=0.05
    )
    ...
    assert numpy.isclose(
        statistic_values[StatisticType.R2], 0.0, rtol=0.05
    )
...
def test_mse():
    ...
    assert numpy.isclose(
        statistic_values[StatisticType.MSE], 0.0, rtol=0.05
    )
