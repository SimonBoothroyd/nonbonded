...
class ImportFreeSolvSchema(ComponentSchema):

    type: Literal["ImportFreeSolv"] = "ImportFreeSolv"


class ImportFreeSolv(Component):
    @classmethod
    ...
    def _apply(
        ...
        for _, row in free_solv_data_frame.iterrows():
            ...
            components = [
                # Solvent
                datasets.Component(smiles="O", mole_fraction=1.0),
                # Solute
                datasets.Component(
            ...
            data_entry = datasets.DataSetEntry(
                property_type="SolvationFreeEnergy",
                temperature=298.15,
                pressure=101.325,
                value=value.to(properties.SolvationFreeEnergy.default_unit()).magnitude,
                std_error=std_error.to(
        ...
        data_rows = [entry.to_series() for entry in data_entries]
        free_solv_data_frame = pandas.DataFrame(data_rows)
